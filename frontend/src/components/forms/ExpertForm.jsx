...
export const ExpertForm = ({ initialData, onSave, onUpdate, onClose, isManager = false }) => {
  ...
  const prepareDataForServer = (data) => {
    return {
      ...
      // --- Данные из ProcessSection ---
    srok_exp: data.deadlineDays ? Number(data.deadlineDays) : null,
    ...
    material_cost: data.materialCosts ? Number(data.materialCosts) : null,
    exploitation_cost: data.equipmentCosts ? Number(data.equipmentCosts) : null,
    full_cost: data.totalCost ? Number(data.totalCost) : null,
    descrip: data.plomba || "",
